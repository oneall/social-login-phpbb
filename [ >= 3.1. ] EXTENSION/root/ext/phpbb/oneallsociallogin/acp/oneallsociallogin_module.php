 ...
 *
 */
namespace phpbb\oneallsociallogin\acp;

class oneallsociallogin_module
{
	...
	public function admin_main ()
	{
		...
		// Add language file.
		// Set up the page
		$this->tpl_name = 'acp_oneallsociallogin';
		...
		// Security Check
		add_form_key ('acp_oneallsociallogin');
		...
		if (isset ($_POST ['submit']))
		{
			...
			// Security Check
			if (! check_form_key ('acp_oneallsociallogin'))
			{
			...
			foreach (oneallsociallogin_module::get_providers () as $provider_key => $provider_data)
			{
		...
		foreach (oneallsociallogin_module::get_providers () as $key => $data)
		{
	...
	public function admin_ajax_verify_api_settings ()
	{
		...
		else
		{
			...
			{
				if (! oneallsociallogin_module::check_fsockopen ($api_connection_use_https))
				{
			...
			{
				if (! oneallsociallogin_module::check_curl ($api_connection_use_https))
				{
					$status_message = 'error|' . $user->lang ['OASL_API_CREDENTIALS_USE_AUTO'];
				}
			}
			...
			if (empty ($status_message))
			{
				...
				else
				{
					...
					$result = oneallsociallogin_module::do_api_request ($api_connection_handler, $api_resource_url, $api_credentials);
	...
	protected function get_user_id_for_login_token ($login_token)
	{
		...
		$sql = "SELECT user_id
						FROM " . $table_prefix . 'oasl_login_token' . "
	 ...
	 */
	function create_login_token_for_user_id ($user_id)
	{
		global $db, $table_prefix;

		// Remove old or existing login token
		$sql = "DELETE FROM " . $table_prefix . 'oasl_login_token' . "
		...
		$sql = "INSERT INTO " . $table_prefix . 'oasl_token' . " " . $db->sql_build_array ('INSERT', $sql_arr);
		$query = $db->sql_query ($sql);
