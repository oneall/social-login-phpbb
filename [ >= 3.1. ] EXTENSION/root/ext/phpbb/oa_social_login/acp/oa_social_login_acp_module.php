...
class oa_social_login_acp_module
{
	...
	public function admin_main ()
	{
		...
		if (isset ($_POST ['submit']))
		{
			...
			foreach (oa_social_login_acp_module::get_providers () as $provider_key => $provider_data)
			{
		...
		foreach (oa_social_login_acp_module::get_providers () as $key => $data)
		{
	...
	public function admin_ajax_verify_api_settings ()
	{
		...
		else
		{
			...
			{
				if (! oa_social_login_acp_module::check_fsockopen ($api_connection_use_https))
				{
			...
			{
				if (! oa_social_login_acp_module::check_curl ($api_connection_use_https))
				{
					$status_message = 'error|' . $user->lang ['OASL_API_CREDENTIALS_USE_AUTO'];
				}
			}
			...
			if (empty ($status_message))
			{
				...
				else
				{
					...
					$result = oa_social_login_acp_module::do_api_request ($api_connection_handler, $api_resource_url, $api_credentials);
